...
def first_timestamp(events):
    users_first_timestamp = events.groupby('user_id').aggregate({'timestamp': 'min'}).reset_index().rename(columns={'timestamp': 'first_timestamp'})
...
def event_aggregation(events, submissions):
    ...
    user_events.action = pd.Categorical(user_events.action, ['discovered', 'viewed', 'started_attempt', 'wrong', 'passed', 'correct'], ordered=True)
...
def drop_duplicates(events):
    # events = events[~((events.duplicated(['user_id', 'step_id', 'action'])) & (events.action.eq('correct')))]
...
def course_complete(events, estimation_param, estimation_value):
    users_course_complete = events.pivot_table(index='user_id', columns='action', values='step_id', aggfunc='count', fill_value=0).reset_index()
...
def metrics_n_days(events, users_course_complete, day_threshold):
    ...
    events = events[events.first_n_days == True]
    users_metrics_n_days = events.pivot_table(index='user_id', columns='action', values='step_id', aggfunc='count', fill_value=0).reset_index()
...
def data_preparation(events, submissions):
    ...
    users_metrics_n_days = metrics_n_days(users_events, users_course_complete, conf.DAY_THRESHOLD)
    return users_metrics_n_days
